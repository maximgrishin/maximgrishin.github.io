...
void onframe() {
	...
	if (y == 8 && 8 <= x && x <= 10) {
		io::sfx(60+x-8, io::Square, x-7);
		io::sfx(64+x-8, io::Triangle, x-7);
	}
	io::cls();
	io::cset(8,8,' ',9,9);
	io::cset(9,8,' ',9,9);
	...
	if (!in) {
		...
		io::sfx(ns[i], io::Square, io::Forte);
	}
